...
#Esto se ejecuta en Bash. Sale command not found asi que ha habido que descargar datasets y dataformat de nuevo desde el terminal en la carpeta bin (antes estaban mal descargados).
...
kable(genomas) 
```
# No iba el kableExtra, así que hemos puesto knitr. KableExtra no es necesario ya que es un paquete cosmético. Necesitamos la función kable, que está dentro del paquete knitr. La función kable_styling no por lo que la borramos. Con ls() podemos comprobar lo que tenemos, en este caso genomas, y con class(genomas) vemos de qué clase es, un data frame. Para conocer su dimensión: dim(genomas), obtenemos que tiene 35 filas y 8 columnas. Los data frames se pueden consultar con [c(7,8),], para ver las filas 7 y 8.
:::{.callout-note icon=false}
## Desafío
1. ¿Qué diferencia hay entre las líneas 7 y 8 de la tabla anterior?
#La línea 7 corresponde a un identificador GCF y la linea 8 corresponde a un identificador GCA, el resto de información es igual, por lo que la lectura, es decir, corresponder al mismo ensamblaje del genoma, pero tienen nombre diferente por una cuestión técnica en la base de datos.

2. Busca una función de R que te permita identificar las líneas repetidas en un
   marco de datos.
#La función es duplicated, y se aplica a vectores, por lo que para identificar las líneas duplicadas: duplicated(genomas). Nos devuelve true/false. En este caso no se encontraron líneas repetidas.
:::
...
```
#Para que saliera habia que encontrar el archivo y cambiar la ruta donde esta. Para ello usamos file.choose y pegamos la ruta correcta.
...
#Al igual que antes, modificamos la ruta y ponemos donde se encuentra nuestro archivo genomic.gff
```{r}
...
# Se obtienen 11304 de la variable CDS, 11304 de la variable exon, 9768 de la variable gene, 10057 de la variable mRNA y 64 de la variable region.
```{r}
