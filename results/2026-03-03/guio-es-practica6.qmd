...
#Como kableExtra nos sigue dando error, utilizamos el paquete knitr, que también contiene la función kable
...
## Para ello, como se trata de una proteina de Swiss-Prot utilizamos reviewed y como lo que queremos es extraer la secuencia:
```{r}
...
## Ahora definimos un objeto para todas las proteínas de UniProt y otro solo para las que tienen asignadas un taxón
```{r}
...
```{r}
Tpro <- fromJSON(paste0(API, 'taxonomy/protein'))
str(Tpro)
Taxconprot <- Taxones[['taxa']][['uniprot']]
Taxconprot
```
:::
# Ahora restamos:
...
## En total, hay 200.859.016 taxones que no tienen asignada proteína.
