...
## únicamente da error el primer bloque, que indica: URL using bad/illegal format or missing URL: URL rejected: Malformed input to a URL function. Tanto el segundo como el tercero funcionan y descargan una tabla. El primer y segundo bloque son muy parecidos, con la diferencia de que el segundo incluye la función URLencode, a la que se le da como argumento una URL y devuelve una dirección muy parecida pero traducida, por ejemplo los espacios %20 o las comillas %22. En cambio, la dirección original sin traducir funciona al pegarla en el navegador, porque los navegadores son lo suficientemente inteligentes para traducir esos espacios o comillas.  Los datos que hemos descargado son una lista de genomas ensamblados que pertenencen al taxon 6157 y son ensamblajes completos (full). 
#Con el último bloque creamos varias variables. Lo que hacemos es usar la función paste0 para concatenar todas esas variables. La desventaja es que hay que escribir bastante, pero en comparación con el bloque anterior, es más visual, además tiene más líneas pero son más cortas, más fáciles de leer. Es muy fácil identificar qué es lo que estamos buscando.
...
# Para saber el número que identifica a Drosophila, buscamos NCBI taxonomy y buscamos Drosophila. Como sabemos que es un género de moscas, seleccionamos ese y obtenemos el taxonomy ID: 7215. 
```{r}
...
#Portal no hace falta porque es lo mismo que el bloque anterior. Hemos ejecutado este bloque pero nos da un archivo vacío, aunque con cabeceras. Al pegar la dirección en el navegador nos da error. Al realizar esta búsqueda avanzada en la web de ENA, vemos que no encuentra resultados buscando por coding sequences pero sí por nucleotidos. Lo modificamos:
```{r}
...
#Ahora sí funciona. Respondemos a las preguntas, para ello usamos la función read.table, dandole nombre:
adh <- read.table('z3.txt', quote = "", header = TRUE)
#Esto da fallo, más columnas que nombres de columnas. Esto es porque no hemos especificado cual es el delimitador de columnas, y toma los espacios, pero en este caso son tabuladores. Lo especificamos.
adh <- read.table('z3.txt', quote = "", header = TRUE, sep = "\t")
#Ahora ya podemos ver la tabla
#1. ¿Cuántas secuencias hay con el término "alcohol dehydrogenase" en su descripción?
#1258 secuencias con el término adh

#2. ¿Qué longitudes tienen? 
#La media de estas longitudes es de 1252 pares de bases. La mediana 839. Esto lo vemos con la función summary(adh$base_count)

#3. ¿De cuántas especies diferentes son estas secuencias?
#De 308 especies. Esto lo podemos ver con: length(unique(adh$scientific_name))

#4. ¿Cuándo se publicaron las más recientes?
#La más reciente es de 2019-06-04.
