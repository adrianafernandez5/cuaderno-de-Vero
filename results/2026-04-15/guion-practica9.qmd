...
## Ahora, los datos han sido cargados en el objeto mtdna (de clase phyDAT).

## Ejercicio 1:

## ¿Puedes confirmar el número de secuencias?

glance(mtdna) #Con esta función podemos confirmar que el número de secuencias (nseq) es 42.También sería posible con la función length(mtdna)

## ¿Qué longitud tiene el alineamiento?

attr(mtdna, "nr") \## El alineamiento tiene 974 sitios.

## ¿Qué función usarías para extraer los nombres de las secuencias?

names(mtdna) \## Con esta función se extraen los nombres de las 42 secuencias.
...
##¿De qué clase son los objetos mt_nj y mt_upgma? class(mt_nj) class(mt_upgma) \## Son de clase phylo.

##Consulta la estructura de estos objetos (str(mt_nj)). str(mt_nj) str(mt_upgma) \## Ambos objetos son árboles filogenéticos de clase phylo. Contienen 42 taxones. El árbol NJ tiene 40 nodos internos, mientras que el árbol UPGMA tiene 41.

##Represéntalos gráficamente (plot(mt_nj)) y consulta la ayuda de la función plot.phylo() para saber qué opciones de representación gráfica tienes. plot(mt_nj) plot(mt_upgma) help(plot.phylo)

##Para qué sirven las funciones add.scale.bar() y axisPhylo()? add.scale.bar(cex = 0.7, font = 2, col = "red") \## Añade una barra de escala que indica la distancia evolutiva.

axisPhylo(side = NULL, root.time = NULL, backward = TRUE) \## Añade un eje numérico que muestra la escala de distancias evolutivas.

## ENRAIZAR

is.rooted(mt_nj) is.rooted(mt_upgma) \## El árbol mt_upgma está enraizado, pero el árbol mt_nj no. Para enraizarlo: nodelabels(mt_nj) mt_nj_rooted \<- root(mt_nj, node = 75)
...
mt_nj_rooted \<- midpoint(mt_nj) plot(mt_nj_rooted, align.tip.label = TRUE, main = 'NJ, enraizado en punto medio') nodelabels(frame = 'none') add.scale.bar()
...
#¿Se corresponden bien las distancias evolutivas estimadas con las dataciones? \# Para responder hay que visualizar la regresión: plot(mt_nj_rtt) \# En general sí corresponden, se encuentran agrupadas las secuencias más antiguas, así como por otro lado las secuencias modernas.

#¿Qué secuencias crees que se ajustan menos a su supuesta edad? \# Las secuencias de neandertales se encuentran muy juntas entre sí a pesar de haber muchos años de diferencia.

#¿Qué razones puede haber para que la edad de una secuencia no se corresponda con la cantidad de cambios que ha acumulado? \# Puede ser debido a errores en las dataciones, puesto que son estimaciones, o como consecuencia del modelo evolutivo que se utilice. En este caso se utilizan métodos heurísticos rápidos, con los que no se usan distancias de alta precisión, simplemente proporciona una idea de la posible relación filogenética, es decir, es una simplificación.
...
modernos \<- startsWith(colnames(distMat), 'H.s.modern') neandertales \<- startsWith(colnames(distMat), 'H.s.neandertal') denisovanos \<- startsWith(colnames(distMat), 'H.s.denisova') chimpances \<- startsWith(colnames(distMat), 'P.troglodytes') bonobos \<- startsWith(colnames(distMat), 'P.paniscus')
...
mean(distMat\[modernos, denisovanos\]) \# La distancia es de 0.0225 mean(distMat\[modernos, chimpances\]) \# La distancia es de 0.0910 mean(distMat\[modernos, bonobos\]) \# La distancia es de 0.0924 mean(distMat\[neandertales, denisovanos\]) \# La distancia es de 0.0220 mean(distMat\[neandertales, chimpances\]) \# La distancia es de 0.0904 mean(distMat\[neandertales, bonobos\]) \# La distancia es de 0.0909 mean(distMat\[denisovanos, chimpances\]) \# La distancia es de 0.0913 mean(distMat\[denisovanos, bonobos\]) \# La distancia es de 0.0930 mean(distMat\[chimpances, bonobos\]) \# La distancia es de 0.0406

# ¿Cuántas veces mayor es la distancia entre humanos y chimpancés que entre humanos y neandertales?

mean(distMat\[modernos, chimpances\])/mean(distMat\[modernos, neandertales\]) \# 7.50 veces mayor.
...
z \<- distMat\[modernos, modernos\] z\[z == 0\] \<- NA mean(z, na.rm = TRUE)
...
plot(distMat, distCofen, xlab = 'Distancias F81', ylab = 'Dist. cofenéticas') abline(a = 0, b = 1, col = 'red', lty = 2)
